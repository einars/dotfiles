...
" repeat-aware function wrappers

function s:input(...)
    if ! exists('g:repeat_executing') || g:repeat_executing == 0
        if a:0 == 2
            let s:last_input = input(a:1, a:2)
        else
            let s:last_input = input(a:1)
        endif
    endif
    return s:last_input
endfunction
...
function! s:process(string)
    ...
    while i < 7
        ...
        if m != ''
            ...
            let repl_{i} = s:input(substitute(m,':\s*$','','').': ')
        endif
    endwhile
    let s = ""
    let i = 0
